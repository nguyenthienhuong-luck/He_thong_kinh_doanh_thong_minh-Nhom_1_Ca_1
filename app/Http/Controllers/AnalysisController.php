...
class AnalysisController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Giới hạn theo các ví thuộc user hiện tại
        ...
        $expenses = Transaction::whereIn('wallet_id', $walletIds)
            ->whereHas('groupType', fn($q) => $q->where('name', 'Khoản chi'))
            ->whereBetween('date', [$startOfMonth, $endOfMonth])
            ->sum('amount');

        $lastExpenses = Transaction::whereIn('wallet_id', $walletIds)
            ->whereHas('groupType', fn($q) => $q->where('name', 'Khoản chi'))
            ->whereBetween('date', [$lastMonthStart, $lastMonthEnd])
            ->sum('amount');

        $pctChange = $lastExpenses > 0 ? round((($expenses - $lastExpenses) / $lastExpenses) * 100, 2) : null;
        ...
        return response()->json([
            'this_month_expense' => $expenses,
            'last_month_expense' => $lastExpenses,
